...
Last updated: 2026-07-11. This is the working state + remaining plan for the next agent.
Read `AGENTS.md`, `ROOT_DOCS.md`, and `CHANGELOG.md` first.

## Current state

- **Backend: `v0.1.93` is built + pushed** (git tag + `ghcr.io/.../spooled-backend:latest`
  both green) and **awaiting a production redeploy**. Prod runs `docker-compose.prod.yml`
...
1. **Verify v0.1.93 live on prod after the redeploy** (blocked until the operator
   redeploys). Re-run the scoped-key containment checks against `https://api.spooled.cloud`
   with a throwaway org: mint-wildcard → 403; read/cancel/workflow/schedule/pause a
   different queue → 403/404; realtime + gRPC scope. Confirm `uptime_seconds` is small first.

2. **F9 — lease fencing by `lease_id`, not `worker_id` (Low, deferred; the one open audit
   finding).** Today complete/fail/heartbeat/renew fence on `assigned_worker_id` +
   `lease_expires_at > NOW()`, never `lease_id`, so a stale completion from a re-leased
   same-worker job can win. A correct fix is cross-repo and must be done end-to-end or it
   breaks all workers (because `lease_id` is never returned to clients today):
   - add a `lease_id` field to the proto `Job` message (`proto/spooled.proto`) and to the
     REST claim response / `ClaimedJob` model;
   - return it from every claim path (REST `queue/mod.rs`, gRPC `queue_service.rs`);
   - require it (`AND lease_id = $x`) in complete/fail/heartbeat/renew on both REST + gRPC;
   - update all 4 SDKs (node/go/python/php) to capture and send `lease_id`, and cut new
     SDK releases. Registries are immutable — new bug = new version; never re-tag.
