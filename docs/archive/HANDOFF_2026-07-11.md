> [!WARNING]
> **Expired historical handoff (2026-07-11).** This document is preserved for context only. It does not describe the current release or production state and grants no authorization to release, deploy, or modify production. Consult the repository's current documentation and workspace operator guidance before acting.
...
- **Rotate `ADMIN_API_KEY`.** Treat any key that appeared in a chat transcript as exposed. Update the deployment secret store and restart/roll the backend; verify old-key rejection.
- **Do not reuse historical tracked `certs/grpc-*.pem`.** Production Compose now generates a private origin cert into a Docker volume (`grpc_tls`). Older image layers that baked `./certs/` remain historical risk.
...
- Prod tests: create throwaway orgs (slugs `zz-*`), and HARD-DELETE them when done
  ...
  tombstone). Never touch real accounts. Load the admin key from the operator's secret
  store / QA env (`SPOOLED_ADMIN_KEY` / `ADMIN_API_KEY`); never print it.
